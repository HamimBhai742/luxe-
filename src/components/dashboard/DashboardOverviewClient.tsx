...
const INITIAL_ORDERS: Order[] = [
  { id: "#ORD-0921", date: "Oct 24, 2023", status: "Processing", total: 129.99 },
  { id: "#ORD-0875", date: "Oct 18, 2023", status: "Shipped", total: 89.50 },
  { id: "#ORD-0812", date: "Oct 05, 2023", status: "Delivered", total: 249.00 },
];
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex justify-between items-start">
          <div className="space-y-3">
            <span className="text-xs font-bold text-zinc-450 dark:text-zinc-500 uppercase tracking-wider block">Total Orders</span>
            <span className="text-3xl font-black text-zinc-900 dark:text-white block leading-none">24</span>
          </div>
          <span className="rounded-xl border border-zinc-100 dark:border-zinc-800 p-2 text-blue-600 dark:text-blue-450">
        ...
        <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex justify-between items-start">
          <div className="space-y-3">
            <span className="text-xs font-bold text-zinc-450 dark:text-zinc-500 uppercase tracking-wider block">Active Shipments</span>
            <span className="text-3xl font-black text-zinc-900 dark:text-white block leading-none">2</span>
          </div>
          <span className="rounded-xl border border-zinc-100 dark:border-zinc-800 p-2 text-blue-600 dark:text-blue-450">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
        ...
        <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex justify-between items-start">
          <div className="space-y-3">
            <span className="text-xs font-bold text-zinc-450 dark:text-zinc-500 uppercase tracking-wider block">Wishlist Items</span>
            <span className="text-3xl font-black text-zinc-900 dark:text-white block leading-none">12</span>
          </div>
          <span className="rounded-xl border border-zinc-100 dark:border-zinc-800 p-2 text-blue-600 dark:text-blue-450">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
        ...
        <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex justify-between items-start">
          <div className="space-y-3">
            <span className="text-xs font-bold text-zinc-450 dark:text-zinc-500 uppercase tracking-wider block">Aura Points</span>
            <span className="text-3xl font-black text-zinc-900 dark:text-white block leading-none">1,250</span>
          </div>
          <span className="rounded-xl border border-zinc-100 dark:border-zinc-800 p-2 text-blue-600 dark:text-blue-450">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
      ...
      <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={() => toast.info("Showing complete orders listing")}
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-850 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              {orders.map((ord) => (
                <tr key={ord.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-850/30 transition-colors">
                  <td className="py-4 pl-3 font-bold text-zinc-900 dark:text-white">{ord.id}</td>
                  <td className="py-4 text-zinc-500 dark:text-zinc-400">{ord.date}</td>
                  <td className="py-4">{renderStatusPill(ord.status)}</td>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {RECOMMENDED_ITEMS.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="group relative flex flex-col bg-white dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-xs hover:shadow-md transition-all duration-300 cursor-pointer"
              >
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wide block">{item.category}</span>
                  ...
                  <div className="flex justify-between items-center mt-4">
                    <span className="text-sm font-extrabold text-zinc-900 dark:text-white">${item.price.toLocaleString()}.00</span>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
